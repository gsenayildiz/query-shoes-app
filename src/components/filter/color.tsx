...
const Color = ({selected, setSelected}: FilterProps) => {
    ...
    useEffect(() => {
        if (selected.length > 0) {
            // seçili elemanları  url e ekle aralarına virgül koy
                ...
                {colors.map((color) => {
                    ...
                    return (
                        <p
                            ...
                            className={`py-2 px-4 lg:px-0 text-center rounded-md cursor-pointer transition hover:bg-zinc-400 text-transparent select-none ${found ? "ring-[4px]" : ""
                                }`}
                        >
                           .
                        </p>
